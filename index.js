...
app.get("/comprehensive_report_division",allowDivisionUsers,async(req,res)=>{
    ...
    const division_site_payment_details_query_result = await db.query("SELECT division_users.division_id,site_users.site_id,site_users.site_name,site_users.khatha_or_property_no,site_users.pid,site_payment_details.* FROM division_users LEFT JOIN site_users ON division_users.division_id = site_users.division_id LEFT JOIN site_payment_details ON site_users.site_id = site_payment_details.site_id WHERE division_users.division_id = $1",[division_id]);
